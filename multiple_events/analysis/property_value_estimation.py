...
periods = dates.to_period('Q').astype(str)

# Check to see if we already performed kriging for this period in a previous run
previous_runs = np.sort([x.split('_')[-1].split('.parquet')[0] for x in os.listdir(outfolder) if 'kriged' in x])

# Drop periods that we ran previously
m = (~periods.isin(previous_runs))
dates = dates[m]
periods = periods[m]
...
# First check to see if cross-validation was already performed as part of a previous run
cv_outname = os.path.join(outfolder,f'group_{group_idx}_cross_validation.parquet')

if not os.path.exists(cv_outname):
    ...
    cv_df = cv_df.drop(columns='geometry')
    cv_df.to_parquet(cv_outname)
    
else:
    print('Cross-validation already performed in a previous run.',flush=True)
...
# Save progress for each period in case it takes multiple runs to complete
for period in periods:
    
    period_points = property_timepoints[property_timepoints['period']==period]

    # To reduce memory consumption, perform kriging in chunks
    chunksize = 250000 - len(sales)
    n_chunks = np.ceil(len(period_points)/chunksize).astype(int)
    
    krig_list = []

    for i,krig_points in enumerate(np.array_split(period_points,n_chunks)):
        
        print(f'\n*** {period}: Chunk {i+1} / {n_chunks} ***\n',flush=True)
    ...
    ### *** SAVE RESULTS *** ###
    kriged_outname = os.path.join(outfolder,f'group_{group_idx}_property_values_kriged_{period}.parquet')
    kriged_df.to_parquet(kriged_outname)

    covfun_outname = os.path.join(outfolder,f'group_{group_idx}_covariance_function.object')
    if not os.path.exists(covfun_outname):
    ...
    if not os.path.exists(variogram_outname):
